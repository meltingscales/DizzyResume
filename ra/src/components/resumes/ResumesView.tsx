...
export function ResumesView() {
  ...
  useEffect(() => {
    // Load the first profile, then fetch its resume variants
    api.profiles
      .list()
      .then((profiles) => {
  ...
  const handleSetDefault = async (variant: ResumeVariant) => {
    try {
      await api.resumes.setDefault(variant.id, variant.profile_id);
      setVariants((prev) =>
        ...
        <div className="space-y-3">
          ...
          {variants.length === 0 && (
            <p className="text-center text-muted-foreground py-8 text-sm">
              No resume variants yet. Create a profile first, then add variants.
            </p>
          )}
